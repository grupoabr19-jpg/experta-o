(function(){
  ...
  meta.conta=['','Minha área','Seu perfil, suas conexões e suas informações na intranet.'];
  meta.feed=['','Feed','Atualizações gerais e publicações por equipe.'];
  cards.push(
    {id:'user-area',section:'conta',audience:'todos',title:'Área do colaborador',tags:['conta','perfil'],html:'<div id="authArea" class="auth-area"><p>Verificando sua sessão...</p></div>'},
  ...
  let pendingPhotoData='',feedScope='general';
  ...
  function feedShell(feed){
    return '<section class="member-feed feed-page"><div class="feed-composer"><span class="section-kicker">FEED #PARCEIRAÇO</span><h3>Compartilhe uma atualização</h3><form id="postForm"><textarea name="content" maxlength="500" rows="4" required placeholder="Informe novidades do time, aprendizados, avisos ou boas práticas..."></textarea><div class="feed-compose-actions"><fieldset class="feed-scope"><label><input type="radio" name="visibility" value="general" '+(feedScope==='general'?'checked':'')+'> Geral</label><label><input type="radio" name="visibility" value="team" '+(feedScope==='team'?'checked':'')+'> Minha equipe</label></fieldset><small>Até 500 caracteres</small><button class="button primary" type="submit">Publicar</button></div></form></div><div class="feed-tabs"><button type="button" data-feed-scope="general" class="'+(feedScope==='general'?'active':'')+'">Feed geral</button><button type="button" data-feed-scope="team" class="'+(feedScope==='team'?'active':'')+'">Minha equipe</button></div><div id="feedList" class="feed-list">'+feedMarkup(feed.posts||[])+'</div></section>';
  }
  ...
  document.addEventListener('input',async event=>{
    ...
    if(event.target.name==='photo'&&event.target.files?.[0]){const file=event.target.files[0];if(file.size>8000000){toast('Escolha uma imagem de até 8 MB.');event.target.value='';return;}try{const bitmap=await createImageBitmap(file),side=Math.min(bitmap.width,bitmap.height),sx=(bitmap.width-side)/2,sy=(bitmap.height-side)/2,canvas=document.createElement('canvas');canvas.width=400;canvas.height=400;canvas.getContext('2d').drawImage(bitmap,sx,sy,side,side,0,0,400,400);pendingPhotoData=canvas.toDataURL('image/jpeg',.86);const preview=document.querySelector('.profile-photo-preview');if(preview)preview.innerHTML='<img src="'+pendingPhotoData+'" alt="Prévia da foto">';toast('Foto ajustada para 400 × 400 px. Salve o perfil para confirmar.');}catch{toast('Não foi possível processar essa imagem.');}}
  });
  ...
  document.addEventListener('submit',async event=>{
    ...
    try{
      ...
      else if(form.id==='profileForm'){await api('/api/profile',{method:'PUT',body:JSON.stringify({...values,photoData:pendingPhotoData||undefined})});pendingPhotoData='';toast('Perfil atualizado.');await renderArea();}
      else{feedScope=values.visibility==='team'?'team':'general';await api('/api/feed',{method:'POST',body:JSON.stringify(values)});toast('Publicação enviada.');await renderFeed();}
