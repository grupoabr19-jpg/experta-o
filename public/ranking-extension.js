(function(){
  ...
  const identity = (item,kind) => kind==='team' ? `<strong>${item.leader||'Líder a definir'}</strong><small>${item.route||item.name||'Região a definir'}</small>` : `<strong>${item.name||'Vendedor a definir'}</strong><small>${item.route||'Região a definir'}</small>`;
  const tons = value => value!=null?Number(value).toLocaleString('pt-BR')+' Ton.':'&mdash;';
  const table = (items,kind) => { const rows=ordered(items); if(!rows.length)return '<div class="ranking-empty">Aguardando dados da planilha de ranking.</div>'; const label=kind==='team'?'Região / equipe':'Vendedor / região'; return `<div class="ranking-table"><div class="ranking-row ranking-head"><span>Pos.</span><span>${label}</span><span>Realizado</span><span>Meta</span><span>% da meta</span><span>Status (%T)</span></div>${rows.map(x=>`<div class="ranking-row"><b>${x.position}&ordm;</b><span>${identity(x,kind)}</span><span>${tons(x.tons)}</span><span>${tons(x.targetTons)}</span><em>${x.attainment.toFixed(1)}%</em><span>${x.status||'&mdash;'}</span></div>`).join('')}</div>`; };
  const populate = data => {
