...
const RESOURCES = ['feed','hero_announcements','profiles','ranking','ranking_ticker','scripts','portfolio','sales_routes','project_war','kommo','blog','marketing','ideaaco','content'];
...
module.exports = function createAdminApi({ db, authenticatedUser, ensureProfile, send, parseBody, cleanText, safeUrl }) {
  ...
  async function me(req, res) {
    ...
    return send(res, 200, { profile: ctx.profile, permissions: ctx.permissions, resources: RESOURCES, isSuperAdmin: ctx.profile.role === 'super_admin', canModerateFeed: allowed(ctx, 'feed', 'manage'), canManageAnnouncements: allowed(ctx, 'hero_announcements', 'manage') });
  }
  async function areas(req, res) {
    ...
    if (req.method === 'GET') {
      ...
      return send(res, 200, { areas: areaRows.rows.map(area => ({ ...area, permissions: Object.fromEntries(permissionRows.rows.filter(p => p.area_id === area.id).map(p => [p.resource, p.access_level])) })), resources: RESOURCES });
    }
    if (req.method === 'POST') {
  ...
  async function moderateFeed(req, res) {
    const ctx = await requireAccess(req, res, 'feed', 'manage'); if (!ctx) return;
